...
class DataGenerator(object):
    ...
    def simulate(self, num_iters, max_num_steps,
            policy=None, policy_idx_type='full', p_diabetes=0.2, modified=False,
        ...
        iter_next_states = np.ones((num_iters, max_num_steps+1, 1), dtype=int)*(-1)
        ...
        for itr in tqdm(range(num_iters), disable=not(use_tqdm), desc=tqdm_desc, leave=False):
            ...
            mdp.state = mdp.get_new_state(modified=modified)
            this_diabetic_idx = mdp.state.diabetic_idx
            iter_component[itr, :] = this_diabetic_idx  # Never changes
            iter_states[itr, 0, 0] = mdp.state.get_state_idx(idx_type=output_state_idx_type) # itr, t=0
            # if the patient dies immediately, then just set the reward as a default to the reward in the current state
            iter_rewards[itr, 0, 0] = np.dot(np.array([mdp.state.get_num_abnormal(), mdp.state.on_treatment()]), np.array([-1, -1.2]))
            for step in range(max_num_steps):
                assert not mdp.state.check_absorbing_state()
                step_action = mdp.select_actions() # selects an action
                ...
                if step_reward != np.dot(np.array([mdp.state.get_num_abnormal(), int(mdp.state.on_treatment())]), np.array([-1, -1.2])):
                    import ipdb; ipdb.set_trace() # All of the data is generated according to the next state category. 
                this_to_state_idx = mdp.state.get_state_idx(idx_type=output_state_idx_type).astype(int)

                iter_actions[itr, step, 0] = this_action_idx # this step's action
                iter_states[itr, step+1, 0] = this_to_state_idx # this is the next state, which is already property of mdp
                ...
                iter_lengths[itr, 0] = step+1
                    # break # This used to mean there was a terminal, now there is not
                if mdp.state.check_absorbing_state(): # If the patient is dead or discharged, finish data collection
                    break
