...
class MDP(object):
    ...
    def __init__(self, init_state_idx=None, init_state_idx_type='obs',
            policy_array=None, policy_idx_type='obs', p_diabetes=0.2, modified=False):
        '''
        ...
        self.state = self.get_new_state(init_state_idx, init_state_idx_type, modified=False)

        self.policy_array = policy_array
        self.policy_idx_type = policy_idx_type  # Used for mapping the policy to actions

    def get_new_state(self, state_idx = None, idx_type = 'obs', diabetic_idx = None, modified = False):
        '''
        ...
        * This option will set glucose to a normal level
        ...
        if option in ['random', 'random_cond_diab'] :
            init_state = self.generate_random_state(diabetic_idx, modified)
    ...
    def generate_random_state(self, diabetic_idx=None, modified=False):
        import joblib
        import os
        _data_dir = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), 'data')
        if not modified:
            isd = joblib.load(os.path.join(_data_dir, 'prior_initial_state_absorbing.joblib'))
        else:
            isd = joblib.load(os.path.join(_data_dir, 'modified_prior_initial_state_absorbing.joblib'))
            isd = (isd > 0)
            isd = isd / isd.sum()
        ...
        if diabetic_idx == 0:
            ...
            s = np.random.choice(np.arange(720), p=prior)
        else:
            prior = isd[720:1440]
            prior = prior / prior.sum()
            s = 720 + np.random.choice(np.arange(720), p=prior)
        
        return State(state_idx=s, idx_type='full')
    
    def generate_random_state_OLD(self, diabetic_idx=None):
        # Note that we will condition on diabetic idx if provided
        if diabetic_idx is None:
            diabetic_idx = np.random.binomial(1, self.p_diabetes)
    ...
    def calculateReward(self):
        # state = [7 covariates]
        # Misspecified: Linear Regression  S x A --> r (MSE is lower)
        # Well specified: Linear Regression [num_abnormal(s'), on_treatment(s')] --> r (MSE is higher)
        # reward is R(s,a) [s, a --> s'] R(s') = \theta * [num_abnormal(s'), on_treatment]
        num_abnormal = self.state.get_num_abnormal() # This is a number between 0 and 4
        # if num_abnormal >= 3:
        #     return -1
        ...
        # Make this function continuous
        reward = -1 * num_abnormal # This is more negative the more num_abnormal that we see
        if self.state.on_treatment():
            reward -= 1.2 # This makes the magnitude of stuff worse
#         reward += np.random.uniform(low=0, high=0.2) 
        return reward
    ...
    def transition_no_effect(self, state, action):
